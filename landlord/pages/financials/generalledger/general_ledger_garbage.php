...
// Force account_id to be '515' for Garbage Income
$account_id = '515';
...
// Always filter for Garbage Income account (account code '515')
$where[] = "jl.account_id = :account_id";
$params[':account_id'] = $account_id;

$whereSql = $where ? "WHERE " . implode(" AND ", $where) : "";

// General Ledger query for Garbage Income only
$sql = "
    SELECT 
        je.created_at,
        je.reference,
...
// Get Garbage Income account name for title
$garbageAccount = $pdo->prepare("SELECT account_name FROM chart_of_accounts WHERE account_code = ?");
$garbageAccount->execute([$account_id]);
$garbageAccountName = $garbageAccount->fetchColumn();
...
$title = "General Ledger - " . htmlspecialchars($garbageAccountName);
?>
    ...
    <aside class="app-sidebar bg-body-secondary shadow" data-bs-theme="dark">
      ...
      <main class="app-main">
        ...
        <div class="app-content-header">
          ...
          <div class="container-fluid">
            <!--begin::Row-->
            <h2 style="color:#FFC107;">General Ledger - Garbage Income</h2>
            
  <!-- Date Filter Form (Only date filters since account is fixed to Garbage Income) -->
        ...
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                ...
                <?php if (empty($ledgerRows)): ?>
                    <tr><td colspan="7" class="text-center text-muted">No garbage income transactions found</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
...
$('#trialBalance tbody').on('click', 'tr[data-account-id]', function () {
  ...
  function exportToExcel() {
    ...
    const wb = XLSX.utils.table_to_book(table, {sheet: "Garbage Income Ledger"});
    XLSX.writeFile(wb, 'Garbage_Income_Ledger_' + new Date().toISOString().split('T')[0] + '.xlsx');
  }
  ...
  function exportToPDF() {
    ...
    doc.text('Garbage Income Ledger Report', 14, 15);
    doc.autoTable({
      html: '#trialBalance',
      startY: 25,
      theme: 'grid',
    ...
    doc.save('Garbage_Income_Ledger_' + new Date().toISOString().split('T')[0] + '.pdf');
  }
  </script>
