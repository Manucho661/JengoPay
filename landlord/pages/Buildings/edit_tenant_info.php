    ...
    <div class="app-wrapper">
        ...
        <div class="container-fluid">
          <?php
            ...
            if(isset($_GET['edit_tenant']) && !empty($_GET['edit_tenant'])) {
              $id = $_GET['edit_tenant'];
              $id = encryptor('decrypt', $id);
              try {
                if(!empty($id)) {
                  $select = $pdo->prepare("SELECT * FROM single_units WHERE id =:id");
                  $select->execute([
                    ':id' => $id
                  ]);
                  $tenant = $select->fetch(PDO::FETCH_ASSOC);
                }
              } catch(Exception $e) {

              }
            }
            include_once '../includes/edit_tenant_form.php';
          ?>
        </div>
      </section>
