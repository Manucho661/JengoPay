...
=======
    ...
    <main class="main">
          ...
          <div class="container-fluid">
            <?php
            // --------------------------------------------
            // COUNT TENANTS BY TENANCY STATUS
            // --------------------------------------------
            $sql = "
            ...
            $counts = [
              'Active'  => 0,
              'Vacated' => 0,
              'Terminated' => 0  // Include if you track terminated tenancies
            ];
            ...
            <div class="row">
              <?php foreach ($counts as $type => $total): ?>
                <div class="col-md-3">
                  <div class="info-box shadow" style="border:1px solid rgba(0,25,45,.3);">
                    <span class="info-box-icon" style="background-color:#00192D; color:#fff;">
                      <i class="bi <?php echo $icons[$type]; ?>"></i>
                    </span>
                    <div class="info-box-content">
                      <span class="info-box-text"><?php echo htmlspecialchars($type); ?></span>
                      <span class="info-box-number"><?php echo (int)$total; ?></span>
>>>>>>> 6be4f45348181e13f4ac604b1111a3d2d2a9938f
                    </div>
                </div>
<<<<<<< HEAD
              ...
              <div class="card-body">
                <table class="table table-striped" id="dataTable">
                  ...
                  <tbody>
                    ...
                    // Query tenants + tenancy + building info
                    $sql = "
                      SELECT 
                          t.id AS tenant_id,
                          t.first_name,
                          ...
                          bu.unit_number,
                          bu.building_id,
                          uc.category_name AS unit_category
                      FROM tenants t
                      JOIN tenancies ten ON ten.tenant_id = t.id
                      JOIN building_units bu ON bu.id = ten.unit_id
                      JOIN unit_categories uc ON uc.id = bu.unit_category_id
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                      $tenantsFound = true;
                      $id = encryptor('encrypt', $row['tenant_id']);
                    ?>
                      <tr>
                        <td><?= htmlspecialchars($row['first_name'] . ' ' . $row['middle_name'] . ' ' . $row['last_name']); ?></td>
                        <td><?= htmlspecialchars($row['unit_number'] . ' (' . $row['building_id'] . ')'); ?></td>
                        <td><?= htmlspecialchars($row['unit_category']); ?></td>
                        <td>
                          <a href="tel:<?= htmlspecialchars($row['main_contact']); ?>"><?= htmlspecialchars($row['main_contact']); ?></a><br>
                          <?php if (!empty($row['alt_phone'])): ?>
                            <a href="tel:<?= htmlspecialchars($row['alt_phone']); ?>"><?= htmlspecialchars($row['alt_phone']); ?></a>
                          <?php endif; ?>
                        </td>
                        <td><i class="bi bi-person-vcard"></i> <?= htmlspecialchars($row['national_id']); ?></td>
                        <td><?= htmlspecialchars($row['move_in_date']); ?></td>
                        <td><?= htmlspecialchars($row['tenant_reg']); ?></td>
                        <td>
                          <?php if ($row['tenancy_status'] === 'Active'): ?>
                            <button class="btn btn-xs shadow" style="background-color:#24953E; color:#fff">
                              <i class="bi bi-person-check"></i> <?= $row['tenancy_status']; ?>
                            </button>
                          <?php else: ?>
                            <button class="btn btn-xs shadow" style="background-color: #cc0001; color:#fff">
                              <i class="bi bi-person-exclamation"></i> <?= $row['tenancy_status']; ?>
                            </button>
                          <?php endif; ?>
                        </td>
                        <td>
                          <div class="btn-group shadow">
                            <button type="button" class="btn btn-default btn-xs" style="border:1px solid rgba(0,25,45,.3);">Action</button>
                            <button type="button" class="btn btn-default dropdown-toggle dropdown-icon btn-xs" data-toggle="dropdown" style="border:1px solid rgba(0,25,45,.3);">
                              <span class="sr-only">Toggle Dropdown</span>
                            </button>
                            <div class="dropdown-menu shadow" role="menu" style="border:1px solid rgba(0,25,45,.3);">
                              <?php if ($row['tenancy_status'] === 'Active'): ?>
                                <a class="dropdown-item" href="<?= 'https://wa.me/' . htmlspecialchars($row['main_contact']) . '/?text=Hello ' . urlencode($row['first_name']); ?>" target="_blank"><i class="fa fa-whatsapp"></i> WhatsApp</a>
                                <a class="dropdown-item" href="messaging.php?message=<?= $id; ?>"><i class="fa fa-envelope"></i> Message</a>
                                <a class="dropdown-item" href="tenant_profile.php?profile=<?= $id; ?>"><i class="fas fa-newspaper"></i> Profile</a>
                                <a class="dropdown-item" href="edit_tenant_info.php?edit_tenant=<?= $id; ?>"><i class="fas fa-edit"></i> Edit</a>
                                <a class="dropdown-item" href="single_unit_tenant_invoice.php?invoice=<?= $id; ?>"><i class="fa fa-newspaper"></i> Invoice</a>
                                <a class="dropdown-item" href="all_individual_tenant_invoices.php?invoice=<?= $id; ?>"><i class="fa fa-table"></i> All Invoices</a>
                                <a class="dropdown-item" data-toggle="modal" data-target="#vacateTenantModal<?= $row['tenant_id']; ?>" href="#"><i class="fa fa-arrow-right"></i> Vacate</a>
                                <a class="dropdown-item" data-toggle="modal" data-target="#shiftTenantModal<?= $row['tenant_id']; ?>" href="#"><i class="fa fa-refresh"></i> Shift</a>
                                <a class="dropdown-item" href="email_tenant.php?email=<?= $id; ?>"><i class="fa fa-envelope-open"></i> Email</a>
                                <a class="dropdown-item" href="tenant_payment.php?payment=<?= $id; ?>"><i class="fas fa-money"></i> Payment</a>
                                <a class="dropdown-item" data-toggle="modal" data-target="#addInfoModal<?= $row['tenant_id']; ?>"><i class="fa fa-plus-square"></i> Add Info</a>
                              <?php else: ?>
                                <a class="dropdown-item" href="tenant_profile.php?profile=<?= $id; ?>"><i class="fas fa-newspaper"></i> Profile</a>
                                <a class="dropdown-item" href="all_individual_tenant_invoices.php?invoice=<?= $id; ?>"><i class="bi bi-receipt"></i> All Invoices</a>
                              <?php endif; ?>
>>>>>>> 6be4f45348181e13f4ac604b1111a3d2d2a9938f
                            </div>
                          </div>
                    ...
                    if (!$tenantsFound) {
                      echo "<tr><td colspan='9' class='text-center'>No tenants found in the database.</td></tr>";
                    }
                    ?>
                  </tbody>
                </table>
